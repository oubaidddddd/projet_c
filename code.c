...
void gerer_client(int a)
{
    if (a==1)
    {
        ...
        if((fwrite != 0)&&(client.Code_cli >= 10000000 && client.Code_cli <= 99999999))
    ...
    else if (a==4)
    {
        ...
        printf("\ncode client      nom           prenom  ");
        for (int c=0 ; c<nbClients ; c++)
        {

            printf("\n_______________________________________________");
            printf("\n%d       ", clients[c].Code_cli);
            printf(" %s         %s", clients[c].Nom, clients[c].Prenom);
            printf("\n_______________________________________________");
        }
...
void gerer_compte(int a)
{
    if (a==1)
    {
        ...
        do{
                printf("Code Compte (4 chiffres): ");
                scanf("%d", &compte.code_cpt);
                if ( compte.code_cpt<= 1000 && compte.code_cpt>= 9999)

                    printf("cette code ne contient pas 4 chiffres\n");

        }while( compte.code_cpt>= 1000 && compte.code_cpt<= 9999);
    ...
    else if (a==3)
    {
        ...
        file = fopen("compte.txt", "r");
        printf("\ncode compte    code client       Date cration       Somme  ");
        printf("\n___________________________________________________________");
        while(fread(&compte, sizeof(struct SCompte), 1, file)) {
                    printf("\n%d", compte.code_cpt);
                    printf("              %d", compte.Code_cli);
                    printf("          %d / %d / %d", compte.d_compte.Jour, compte.d_compte.Mois, compte.d_compte.Annee);
                    printf("       %d", compte.somme);
                    printf("\n___________________________________________________________");
        }
        fclose(file);
    }
...
{
    system("cls");
    printf("\t\t|                    PROJECT PROGRAMMATION C                |\n");
	printf("\t\t|           GESTION D'UNE AGENCE BANCAIRE (E-BANK)          |\n");
    printf("\t\t|                            By MAHFOUDHI OUBAID            |\n");
    printf("\t\t|                            CLASE:II1A                     |\n");
    printf("\t\t-------------------------------------------------------------\n");
    menu();
    return 0;
